  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setPrazoConcedidoDias(data.prazo_concedido_dias != null ? String(data.prazo_concedido_dias) : "");
        ...
        setAcompanhamentoEspecial(data.acompanhamento_especial == null ? "" : data.acompanhamento_especial ? "Sim" : "Não");
      } catch (error) {
        if (!active) return;
    ...
    <AppLayout>
      ...
      <form className="space-y-5 max-w-6xl pb-6" onSubmit={handleSubmit}>
        ...
        <SectionCard title="Tramitação Judicial" subtitle="Acompanhamento da fase judicial, decisão e cumprimento.">
          ...
          {exibeDataEnvioEVara && <Field label="Vara / Juízo" value={varaJuizo} onChange={(e) => setVaraJuizo(e.target.value)} />}
          ...
          {exibeDecisaoEObservacoes && <TextArea label="Observações da decisão" value={observacoesDecisao} onChange={(e) => setObservacoesDecisao(e.target.value)} />}
        </SectionCard>
        ...
        <SectionCard title="Controle Interno" subtitle="Priorização e acompanhamento interno da unidade.">
          ...
          <Field label="Equipe responsável" value={equipeResponsavel} onChange={(e) => setEquipeResponsavel(e.target.value)} />
