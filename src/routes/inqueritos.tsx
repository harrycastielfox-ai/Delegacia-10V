...
{filtered.map((row) => {
  ...
  return <tr key={row.id} className="min-h-[84px] border-t border-border/70 align-middle transition hover:bg-muted/20"><td className="px-4 py-4 align-middle"><div className="flex min-h-[52px] items-center"><p className="truncate font-semibold text-primary">{row.numeroPpe || FALLBACK}</p></div></td><td className="px-4 py-4 align-middle"><div className="flex min-h-[52px] items-center"><p className="line-clamp-2 break-words leading-5 text-foreground/95" title={row.tipificacao}>{row.tipificacao || FALLBACK}</p></div></td><td className="px-4 py-4 align-middle"><div className="flex min-h-[52px] items-center"><p className="line-clamp-2 break-words leading-5" title={row.vitima}>{row.vitima || FALLBACK}</p></div></td><td className="px-4 py-4 align-middle"><div className="flex min-h-[52px] items-center justify-center"><span className={`inline-flex min-h-6 items-center justify-center rounded-full border px-2.5 py-1 text-[10px] font-bold tracking-wide ${priorTone[row.prioridade] ?? ""}`}>{row.prioridade || FALLBACK}</span></div></td><td className="px-4 py-4 align-middle"><div className="flex min-h-[52px] items-center justify-center"><span className="inline-flex max-w-full items-center justify-center truncate rounded-full border border-info/30 bg-info/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-info">{row.gravidade || FALLBACK}</span></div></td><td className="px-4 py-4 align-middle"><div className="flex min-h-[52px] items-center justify-center"><span className={`inline-flex min-h-6 items-center justify-center rounded-full border px-2.5 py-1 text-[10px] font-bold tracking-wide ${statusTone[situacao] ?? ""}`}>{situacao || FALLBACK}</span></div></td><td className="px-4 py-4 align-middle"><div className="flex min-h-[52px] items-center"><p className="line-clamp-2 break-words text-xs text-muted-foreground">{row.equipe || FALLBACK}</p></div></td><td className="px-4 py-4 align-middle"><div className="flex min-h-[52px] items-center justify-center"><span className={`inline-flex min-h-6 items-center justify-center rounded-md border px-2 py-1 text-xs font-semibold ${prazoVencido ? "border-destructive/40 bg-destructive/10 text-destructive" : "border-border bg-background text-foreground"}`}>{row.prazo || FALLBACK}</span></div></td><td className="px-4 py-4 align-middle"><div className="flex min-h-[52px] items-center justify-center"><button className="inline-flex min-h-8 items-center justify-center rounded-lg border border-info/40 bg-info/15 px-3.5 py-1.5 text-xs font-semibold text-info transition hover:bg-info/25 hover:text-info/90" onClick={() => navigate({ to: "/inqueritos/$caseId", params: { caseId: row.id } })}>Abrir</button></div></td></tr>;
})} 
