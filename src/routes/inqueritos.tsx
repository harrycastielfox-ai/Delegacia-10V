...
import { calculateInqueritoOperationalPriority, normalizeCaseCategory } from "@/lib/inqueritosPriority";
...
type InqueritoListRow = { source: InqueritoRecord; id: string; numeroPpe: string; tipificacao: string; vitima: string; prioridade: string; gravidade: string; tipoProcedimento: string; bairro: string; situacao: string; statusDiligencias: string; equipe: string; escrivao: string; prazo: string; investigado: string; reuPreso: string; custodia: string; medidaProtetiva: string; diligenciasPendentes: string; protetivaTexto: string; dataFato: string; dataInstauracao: string; dataEnvioRelatorio: string; createdAt: string; relatorioEnviado: string; elucidado: string; motivacao: string; fullText: string; };
...
function isFalseyLike(value: unknown) { const n = normalizeText(String(value ?? "")).replace(/[-_]/g, " "); return ["false","f","0","nao","n","no","pendente","nao elucidado"].includes(n); }
...
function hasTextValue(value: string) { return Boolean(normalizeText(value)) && value !== FALLBACK; }
function getProcedureType(value: string) { const text = normalizeText(value); if (!text) return ""; if (text === "ip" || text.includes("inquerito policial")) return "IP"; if (text === "apf" || text.includes("flagrante")) return "APF"; if (text === "tco" || text.includes("termo circunstanciado")) return "TCO"; if (text === "boc" || text.includes("boletim de ocorrencia")) return "BOC"; if (text === "aiai" || text.includes("ato infracional")) return "AIAI"; return value.trim(); }
function isSemRelatorio(row: InqueritoListRow) { return !isTruthyLike(row.relatorioEnviado) && !hasTextValue(row.dataEnvioRelatorio); }
function isConcluidoCentral(row: InqueritoListRow) { return isTruthyLike(row.relatorioEnviado) || hasTextValue(row.dataEnvioRelatorio); }
function isEmAndamentoCentral(row: InqueritoListRow) { const situacao = normalizeText(row.situacao); return !isConcluidoCentral(row) && !["arquiv", "finaliz", "encerr"].some((term) => situacao.includes(term)); }
function getCategoryText(row: InqueritoListRow) { return normalizeText(`${row.gravidade} ${row.tipificacao} ${row.tipoProcedimento} ${row.motivacao}`); }
function isCvli(row: InqueritoListRow) { return isCvliRecord(row.source); }
function isPatrimonial(row: InqueritoListRow) { return ["patrimonio", "furto", "roubo", "receptacao", "estelionato", "dano"].some((term) => getCategoryText(row).includes(term)); }
function isViolenciaDomestica(row: InqueritoListRow) { return ["violencia domestica", "maria da penha"].some((term) => getCategoryText(row).includes(term)); }
function isDrogas(row: InqueritoListRow) { return ["drogas", "trafico", "entorpecente"].some((term) => getCategoryText(row).includes(term)); }
function isSexual(row: InqueritoListRow) { return ["sexual", "estupro", "importunacao"].some((term) => getCategoryText(row).includes(term)); }
function isMiae(row: InqueritoListRow) { return ["miae", "morte por intervencao"].some((term) => getCategoryText(row).includes(term)); }
function isViolento(row: InqueritoListRow) { return ["violento", "lesao corporal", "ameaca"].some((term) => getCategoryText(row).includes(term)); }
function isCriancaAdolescente(row: InqueritoListRow) { return ["crianca", "adolescente", "eca"].some((term) => getCategoryText(row).includes(term)); }
function isTransito(row: InqueritoListRow) { return ["transito", "ctb", "direcao perigosa", "embriaguez"].some((term) => getCategoryText(row).includes(term)); }
function getEntryDate(row: InqueritoListRow) { return parseAnyDate(row.dataInstauracao) ?? parseAnyDate(row.createdAt) ?? parseAnyDate(row.dataFato); }
function getReportDate(row: InqueritoListRow) { return parseAnyDate(row.dataEnvioRelatorio); }
function dateToUtcDay(value: Date | null) { if (!value) return null; return Date.UTC(value.getUTCFullYear(), value.getUTCMonth(), value.getUTCDate(), 12, 0, 0, 0); }
function getCvliDate(row: InqueritoListRow) { return dateToUtcDay(getCvliReferenceDate(row.source)); }
function getDateByField(row: InqueritoListRow, field: string) { const normalized = normalizeText(field); if (normalized === "relatorio") return getReportDate(row); if (normalized === "cvli") return getCvliDate(row); return getEntryDate(row); }
function matchesCategory(row: InqueritoListRow, category: string) { const normalized = normalizeText(category); if (normalized === "cvli") return isCvli(row); if (normalized === "patrimonial" || normalized === "patrimoniais") return isPatrimonial(row); if (normalized === "violencia_domestica" || normalized === "violencia domestica") return isViolenciaDomestica(row); if (normalized === "drogas") return isDrogas(row); if (normalized === "sexual" || normalized === "sexuais") return isSexual(row); if (normalized === "miae") return isMiae(row); if (normalized === "violento") return isViolento(row); if (normalized === "crianca_adolescente" || normalized === "crianca adolescente") return isCriancaAdolescente(row); if (normalized === "transito") return isTransito(row); if (normalized === "outros") return !isCvli(row) && !isPatrimonial(row) && !isViolenciaDomestica(row) && !isDrogas(row) && !isSexual(row) && !isMiae(row) && !isViolento(row) && !isCriancaAdolescente(row) && !isTransito(row); return true; }
...
function normalizeInqueritoForList(caso: InqueritoRecord): InqueritoListRow {
  ...
  return { source: caso, id: caso.id, numeroPpe: pick(raw, "numero_ppe", "numeroPpe", "ppe"), tipificacao: pick(raw, "tipificacao", "classificacao", "tipo_penal"), vitima: pick(raw, "vitima", "vítima"), prioridade: pick(raw, "prioridade"), gravidade: normalizeCaseCategory(pick(raw, "categoria_caso", "categoriaCaso", "gravidade")), tipoProcedimento: pick(raw, "tipo", "tipo_procedimento", "tipoProcedimento", "procedimento"), bairro: pick(raw, "bairro", "localidade", "local", "comunidade"), situacao: pick(raw, "situacao", "situação", "status"), statusDiligencias: pick(raw, "status_diligencias", "statusDiligencias"), equipe: pick(raw, "equipe"), escrivao: pick(raw, "escrivao", "escrivao_responsavel", "responsavel", "responsavel_escrivao"), prazo: pick(raw, "prazo", "data_prazo"), investigado: pick(raw, "investigado", "suspeito", "autor_investigado", "autorInvestigado"), reuPreso: pick(raw, "reu_preso", "reuPreso"), custodia: pick(raw, "custodia", "situacao_custodia"), medidaProtetiva: pick(raw, "medida_protetiva", "medidaProtetiva", "medidas_protetivas", "medidasProtetivas", "protetiva"), diligenciasPendentes: pick(raw, "diligencias_pendentes", "diligenciasPendentes"), protetivaTexto: [pick(raw, "tipo", "tipificacao", "classificacao", "tipo_penal"), pick(raw, "medida_protetiva", "medidaProtetiva", "medidas_protetivas", "medidasProtetivas", "protetiva")].map(normalizeText).join(" "), dataFato: pick(raw, "data_fato", "dataFato"), dataInstauracao: pick(raw, "data_instauracao", "dataInstauracao"), dataEnvioRelatorio: pick(raw, "data_envio_relatorio", "dataEnvioRelatorio"), createdAt: pick(raw, "created_at", "createdAt"), relatorioEnviado: pick(raw, "relatorio_enviado", "relatorioEnviado"), elucidado: pick(raw, "elucidado"), motivacao: pick(raw, "motivacao", "motivação"), fullText: fields.join(" ") };
}
...
function Inqueritos() {
  ...
  const [situacaoFilter, setSituacaoFilter] = useState("todos"); const [prioridadeFilter, setPrioridadeFilter] = useState("todos"); const [gravidadeFilter, setGravidadeFilter] = useState("todos"); const [equipeFilter, setEquipeFilter] = useState("todos"); const [escrivaoFilter, setEscrivaoFilter] = useState("todos"); const [tipoFilter, setTipoFilter] = useState("todos"); const [prazoFilter, setPrazoFilter] = useState("todos"); const [dataInicial, setDataInicial] = useState(""); const [dataFinal, setDataFinal] = useState(""); const [dataFieldFilter, setDataFieldFilter] = useState("entrada"); const [relatorioFilter, setRelatorioFilter] = useState("todos"); const [categoriaFilter, setCategoriaFilter] = useState("todos"); const [statusQueryFilter, setStatusQueryFilter] = useState("todos"); const [elucidadoFilter, setElucidadoFilter] = useState(false); const [naoElucidadoFilter, setNaoElucidadoFilter] = useState(false); const [reuPresoFilter, setReuPresoFilter] = useState(false); const [reuPresoStrictFilter, setReuPresoStrictFilter] = useState(false); const [medidaProtetivaFilter, setMedidaProtetivaFilter] = useState(false); const [diligenciasPendentesFilter, setDiligenciasPendentesFilter] = useState(false);
  ...
  useEffect(() => {
    ...
    setSearchTerm(""); setSituacaoFilter("todos"); setPrioridadeFilter("todos"); setGravidadeFilter("todos"); setEquipeFilter("todos"); setEscrivaoFilter("todos"); setTipoFilter("todos"); setPrazoFilter("todos"); setDataInicial(""); setDataFinal(""); setDataFieldFilter("entrada"); setRelatorioFilter("todos"); setCategoriaFilter("todos"); setStatusQueryFilter("todos"); setElucidadoFilter(false); setNaoElucidadoFilter(false); setReuPresoFilter(false); setReuPresoStrictFilter(false); setMedidaProtetivaFilter(false); setDiligenciasPendentesFilter(false);
    assign("prioridade", setPrioridadeFilter); assign("situacao", setSituacaoFilter); assign("gravidade", setGravidadeFilter); assign("equipe", setEquipeFilter);
    const escrivaoParam = params.get("escrivao"); if (escrivaoParam?.trim()) setEscrivaoFilter(escrivaoParam === "__sem_escrivao" ? EMPTY_FILTER : escrivaoParam);
    const tipoParam = params.get("tipo"); if (tipoParam?.trim()) setTipoFilter(getProcedureType(tipoParam) || tipoParam);
    const relatorio = normalizeText(params.get("relatorio") ?? "todos").replace(/-/g, "_"); if (["todos","sem_relatar","enviado","relatado_nao_enviado"].includes(relatorio)) setRelatorioFilter(relatorio);
    const categoria = normalizeText(params.get("categoria") ?? "todos").replace(/-/g, "_"); if (["todos","cvli","patrimonial","patrimoniais","violencia_domestica","drogas","sexual","sexuais","miae","violento","crianca_adolescente","transito","outros"].includes(categoria)) setCategoriaFilter(categoria);
    const dataCampo = normalizeText(params.get("dataCampo") ?? "entrada").replace(/-/g, "_"); if (["entrada","relatorio","cvli"].includes(dataCampo)) setDataFieldFilter(dataCampo);
    const status = normalizeText(params.get("status") ?? "").replace(/-/g, "_"); if (["em_andamento"].includes(status)) setStatusQueryFilter(status);
    ...
    setReuPresoFilter(reuPresoStrict || isTruthyLike(params.get("reuPreso") ?? "") || isTruthyLike(params.get("custodia") ?? ""));
  ...
  const escrivaoOptions=useMemo(()=>options(normalizedRows.map((r)=>r.escrivao)),[normalizedRows]);
  ...
  const filtered = useMemo(() => normalizedRows.filter((r) => {
    ...
    if (query && !(r.fullText.includes(query) || (isFallbackSearch && [r.numeroPpe,r.tipificacao,r.vitima,r.investigado,r.prioridade,r.gravidade,r.situacao,r.statusDiligencias,r.equipe,r.escrivao,r.prazo].some(isEmpty)))) return false;
    ...
    if (normalizeText(escrivaoFilter) !== "todos" && !(escrivaoFilter===EMPTY_FILTER ? isEmpty(r.escrivao) : normalizeText(r.escrivao)===normalizeText(escrivaoFilter))) return false;
    if (normalizeText(tipoFilter) !== "todos" && !(tipoFilter===EMPTY_FILTER ? isEmpty(r.tipoProcedimento) : getProcedureType(r.tipoProcedimento)===getProcedureType(tipoFilter))) return false;
    if (normalizeText(categoriaFilter) !== "todos" && !matchesCategory(r, categoriaFilter)) return false;
    ...
    if (normalizeText(relatorioFilter) === "relatado_nao_enviado" && !(matchesSituacaoAlias(r.situacao, "relatado") && isSemRelatorio(r))) return false;
    if (normalizeText(statusQueryFilter) === "em_andamento" && !isEmAndamentoCentral(r)) return false;
    ...
    if (reuPresoFilter && !(reuPresoStrictFilter ? isTruthyLike(r.reuPreso) : hasReuPreso(r))) return false;
    ...
    const hasDateFilter = Boolean(dataInicial || dataFinal); if (hasDateFilter) { const ts=getDateByField(r, dataFieldFilter); if(ts===null) return false; const start=parseDateToUtc(dataInicial,false); const end=parseDateToUtc(dataFinal,true); if(start!==null && ts<start) return false; if(end!==null && ts>end) return false; }
    return true;
  }), [normalizedRows, searchTerm, situacaoFilter, prioridadeFilter, gravidadeFilter, equipeFilter, escrivaoFilter, tipoFilter, prazoFilter, dataInicial, dataFinal, dataFieldFilter, relatorioFilter, categoriaFilter, statusQueryFilter, elucidadoFilter, naoElucidadoFilter, reuPresoFilter, reuPresoStrictFilter, medidaProtetivaFilter, diligenciasPendentesFilter]);

  const hasActiveFilters = Boolean(searchTerm.trim() || [situacaoFilter, prioridadeFilter, gravidadeFilter, equipeFilter, escrivaoFilter, tipoFilter, prazoFilter, relatorioFilter, categoriaFilter, statusQueryFilter].some((f) => normalizeText(f) !== "todos") || dataInicial || dataFinal || elucidadoFilter || naoElucidadoFilter || reuPresoFilter || medidaProtetivaFilter || diligenciasPendentesFilter);
    ...
    <tbody>
      ...
      {filtered.map((row) => {
        ...
        return <tr key={row.id} className="border-t border-border/70 align-middle transition hover:bg-muted/20">
          ...
          <td className="px-2 py-2.5 text-center align-middle"><button type="button" onClick={()=>setGravidadeFilter(isEmpty(row.gravidade) ? EMPTY_FILTER : row.gravidade)} className="mx-auto block max-w-full truncate text-center text-xs font-medium leading-5 text-muted-foreground hover:text-foreground/80" title={gravidade}>{gravidade}</button></td>
