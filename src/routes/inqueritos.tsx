...
function Inqueritos() {
  ...
  const [rows, setRows] = useState(INQUERITOS_CASOS);
  const [selected, setSelected] = useState<(typeof INQUERITOS_CASOS)[number] | null>(null);
  const [editing, setEditing] = useState<(typeof INQUERITOS_CASOS)[number] | null>(null);
  ...
  return <AppLayout><PageHeader title="Inquéritos" subtitle={`${PANORAMA.totalCadastrados} procedimentos cadastrados — ${PANORAMA.relatorioEnviado} concluídos`} />
    ...
    <div className="bg-card border border-border rounded-xl overflow-auto"><table className="w-full text-sm min-w-[1200px]"><thead className="bg-muted/40 text-[10px] tracking-[0.15em] text-muted-foreground"><tr><th className="text-left px-4 py-3 font-bold">ID</th><th className="text-left px-4 py-3 font-bold">Nº PPE</th><th className="text-left px-4 py-3 font-bold">PRIOR.</th><th className="text-left px-4 py-3 font-bold">TIPIFICAÇÃO</th><th className="text-left px-4 py-3 font-bold">GRAVIDADE</th><th className="text-left px-4 py-3 font-bold">TIPO</th><th className="text-left px-4 py-3 font-bold">BAIRRO</th><th className="text-left px-4 py-3 font-bold">RÉU PRESO</th><th className="text-left px-4 py-3 font-bold">STATUS</th><th className="text-right px-4 py-3 font-bold">DIAS</th><th className="text-right px-4 py-3 font-bold">AÇÃO</th></tr></thead><tbody>{filtered.map((r) => <tr key={r.id} className="border-t border-border hover:bg-muted/20"><td className="px-4 py-3 font-semibold">{r.id}</td><td className="px-4 py-3 font-semibold">{r.ppe}</td><td className="px-4 py-3"><span className={`text-[10px] font-bold px-2 py-1 rounded border ${priorTone[r.prioridade]}`}>{r.prioridade}</span></td><td className="px-4 py-3 text-xs">{r.tipificacao}</td><td className="px-4 py-3 text-xs">{r.gravidade}</td><td className="px-4 py-3 text-xs">{r.tipo}</td><td className="px-4 py-3 text-xs">{r.bairroDistrito}</td><td className="px-4 py-3 text-xs">{r.reuPreso ? "SIM" : "—"}</td><td className="px-4 py-3"><span className={`text-[10px] font-bold px-2 py-1 rounded border ${statusTone[r.statusDiligencias] ?? ""}`}>{r.statusDiligencias.toUpperCase()}</span></td><td className="px-4 py-3 text-right text-xs">{r.diasCorridos}d</td><td className="px-4 py-3 text-right"><div className="inline-flex gap-2"><button className="rounded-md border border-info/30 bg-info/10 px-2.5 py-1 text-[11px]" onClick={() => setSelected(r)}>Abrir</button><button className="rounded-md border border-border bg-card px-2.5 py-1 text-[11px]" onClick={() => setEditing({ ...r })}>Editar</button><button className="rounded-md border border-destructive/30 bg-destructive/10 px-2.5 py-1 text-[11px] text-destructive" onClick={() => { if (confirm("Deseja remover este inquérito da lista local? No sistema final, esta ação deverá ser registrada em auditoria.")) { setRows((prev) => prev.filter((x) => x.id !== r.id)); setFeedback("Inquérito removido localmente. Exclusão definitiva dependerá de auditoria e backend."); } }}>Excluir</button></div></td></tr>)}</tbody></table></div>
    {feedback && <div className="mt-3 text-sm text-success">{feedback}</div>}
    {selected && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4"><div className="w-full max-w-xl bg-card border border-border rounded-xl p-5"><h3 className="font-bold mb-3">Detalhes do Inquérito</h3><p className="text-sm">{selected.id} — PPE {selected.ppe} — {selected.tipificacao}</p><button className="mt-4 px-3 py-2 text-xs border border-border rounded-md" onClick={() => setSelected(null)}>Fechar</button></div></div>}
    {editing && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4"><div className="w-full max-w-xl bg-card border border-border rounded-xl p-5"><h3 className="font-bold mb-3">Editar Inquérito</h3><input value={editing.tipificacao} onChange={(e) => setEditing((p) => p ? { ...p, tipificacao: e.target.value } : p)} className="w-full bg-card border border-border rounded-md px-3 py-2 text-sm" /><div className="mt-4 flex justify-end gap-2"><button className="px-3 py-2 text-xs border border-border rounded-md" onClick={() => setEditing(null)}>Cancelar</button><button className="px-3 py-2 text-xs rounded-md bg-primary text-primary-foreground" onClick={() => { if (!editing) return; setRows((prev) => prev.map((x) => x.id === editing.id ? editing : x)); setEditing(null); setFeedback("Inquérito atualizado localmente. Integração com banco será feita futuramente."); }}>Salvar alterações</button></div></div></div>}
  </AppLayout>;
}
