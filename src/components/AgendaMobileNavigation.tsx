...
import { CalendarDays, ListOrdered, LogOut } from "lucide-react";
...
export function AgendaMobileNavigation({ profile }: { profile: UserProfile }) {
    ...
    <>
      ...
      >
        <div className="mx-auto grid min-h-16 max-w-lg grid-cols-2 items-stretch">
          {items.map((item) => {
            const active =
              item.url === "/agenda" ? pathname === item.url : pathname.startsWith(item.url);
