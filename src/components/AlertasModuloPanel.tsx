...
export function AlertasModuloPanel({ modulo }: { modulo: ModuleKey }) {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [moduleAlerts, setModuleAlerts] = useState<Record<ModuleKey, SmartAlert[]> | null>(null);
  ...
  const detailedAlerts = useMemo(
    () => (moduleAlerts ? moduleAlerts[modulo] : []),
    [moduleAlerts, modulo],
  );
  const meta = moduleMeta[modulo];
  const visual = moduleVisual[modulo];
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [search, entityFilter, teamFilter, dueFilter, sortBy, pageSize, modulo]);
  ...
  const canOpenAlert = (alert: SmartAlert) => {
    ...
    const isSigilosaAlert = modulo === "sigilosas";
        ...
        <div className="flex flex-col gap-5 p-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              <h2 className="text-xl font-semibold text-foreground sm:text-2xl">{meta.title}</h2>
                  ...
                  {pagedAlerts.map((alert) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0 pr-4">
                          <p className="line-clamp-2 text-sm text-foreground">{alert.principal}</p>
                        ...
                        <div className="min-w-0 pr-4">
                          <p className="line-clamp-2 text-sm text-foreground">{alert.typeLabel}</p>
