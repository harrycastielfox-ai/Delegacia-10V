    ...
    error: {
      ...
      status:
        typeof maybeError.status === "number" || typeof maybeError.status === "string"
          ...
          : null,
      statusCode:
        typeof maybeError.statusCode === "number" || typeof maybeError.statusCode === "string"
...
): Promise<{ data: AuditoriaEvent[]; error: RpcErrorDetails | null }> {
...
export async function listAuditoriaForAdminUser(
  userId: string,
  options?: ListAuditoriaOptions,
): Promise<{ data: AuditoriaEvent[]; error: RpcErrorDetails | null }> {
  ...
  try {
    ...
    if (error) {
      ...
      return {
        data: [],
        error: getRpcError(error, "Erro inesperado ao listar auditoria individual"),
      };
    }
    return { data: (data ?? []) as AuditoriaEvent[], error: null };
  } catch (error) {
