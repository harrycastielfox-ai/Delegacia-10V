...
export type ModuleKey = "criticos" | "prazos" | "operacionais" | "judiciais" | "dados-incompletos" | "sigilosas";
...
export const moduleMeta: Record<ModuleKey, { title: string; desc: string; badge: string }> = {
  criticos: { title: "Alertas Críticos", desc: "Situações que exigem ação imediata e prioridade máxima.", badge: "Urgência" },
  prazos: { title: "Alertas de Prazo", desc: "Prazos vencidos ou críticos que requerem atenção.", badge: "Temporal" },
  operacionais: { title: "Alertas Operacionais", desc: "Ocorrências, diligências e tarefas operacionais pendentes.", badge: "Execução" },
  judiciais: { title: "Alertas Judiciais", desc: "Decisões, intimações e comunicações judiciais pendentes.", badge: "Forense" },
  "dados-incompletos": { title: "Dados Incompletos", desc: "Registros com informações incompletas ou inconsistentes.", badge: "Qualidade" },
  sigilosas: { title: "Representações Sigilosas", desc: "Representações sigilosas com pendências de análise.", badge: "Acesso restrito" },
};

export const normalizeText = (v?: string | null) => (v ?? "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim();
...
export function buildSmartAlerts(inqueritos: InqueritoRecord[], representacoes: RepresentacaoRecord[]) {
